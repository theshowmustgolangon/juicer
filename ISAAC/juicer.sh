...
#SBATCH -o /lustre/isaac/proj/UTHSC0013/panjun_work/projects/hic/2022_C/hic_analysis/juicer/%j.log
...
BASE_LOCATION='/lustre/isaac/proj/UTHSC0013/panjun_work/projects/hic/2022_C/hic_analysis/juicer'

# can also be changed via the -C flag : we don't split
# splitsize=90000000
...
juiceDir=$(BASE_LOCATION)
topDir=$(BASE_LOCATION)
...
then
    ...
    final) final=1 ;;
	postproc) postproc=1 ;; 
    *)  echo "usageHelp>>>>>> $usageHelp"
	    echo "stageHelp>>>>>> $stageHelp"
	    exit 1
    esac
fi
...
refSeq=$BASE_LOCATION/references/rn7chr.fa
genomePath=/lustre/isaac/proj/UTHSC0013/panjun_work/refs/rn7_ucsc/bwa_rn7chr/rn7chr.fa
...
# manual site file
site_file=$BASE_LOCATION/restriction_sites/rn7chr_arima_juicer.txt
...
fastqdir=${topDir}/fastq/${SAMPLE}/*_R*.fastq*
...
then
    ...
    if [ ! -d "$topDir/fastq/$SAMPLE" ]; then
	echo "Directory \"$topDir/fastq/$SAMPLE\" does not exist."
	echo "Create \"$topDir/fastq/$SAMPLE\" and put fastq files to be aligned there."
	...
	then
	    ...
	    if [ ! -d "$splitdir" ]; then 
		echo "***! Failed to find any files matching ${fastqdir}" #orginal or at least splitted file should be exist
		echo "***! Type \"juicer.sh -h \" for help"
		exit 1		
	    fi
	fi
...
########################################################################################
## Create aligned output directory, only if not in postproc, dedup or final stages #####
########################################################################################
		...
		then
		   ...
		   then
		      ...
		      $call_bwameth $threadstring -5SP --do-not-penalize-chimeras --read-group '$rg' --reference ${refSeq} $name1$ext $name2$ext > $name$ext.sam      
		   	...
		   	################################################
			## our case: paried-end and NOT methylation
		   	################################################
